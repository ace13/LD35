...
namespace sf
{
	...
	void opNeg(asIScriptGeneric* gen)
	{
		sf::Vector2f* a = (sf::Vector2f*)gen->GetObject();

		new (gen->GetAddressOfReturnLocation()) sf::Vector2f(-*a);
	}
}
}
...
void as::priv::RegVec2(asIScriptEngine* eng)
{
	...
	AS_ASSERT(eng->RegisterObjectMethod("Vec2", "Vec2 opNeg() const", asFUNCTION(sf::opNeg), asCALL_GENERIC));
